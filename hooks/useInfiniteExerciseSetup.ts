...
interface UseInfiniteExerciseSetupReturn {
  ...
  ) => Promise<ExerciseLibraryResponse[] | undefined>;
...
export const useInfiniteExerciseSetup = (
  filters: ExerciseLibraryFilters = {},
): UseInfiniteExerciseSetupReturn => {
  // Key generator function for useSWRInfinite
  const getKey = (
    pageIndex: number,
    previousPageData: ExerciseLibraryResponse | null,
  ) => {
    ...
    return `/api/exercise-setup?${queryString}`;
  };
  ...
  const isReachingEnd = useMemo(() => {
    ...
    const limit = filters.limit || 12;
    return (
      lastPage.data.length < limit || exercises.length >= (meta?.total || 0)
    );
